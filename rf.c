    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      case DT_DIR:
        if (is_child(entry->d_name) &&
          ...
          if (recurse_find(patterns, pattern_count, child_path, switches)) {
            break_early = 1;
            break;
          };
        }
      ...
      case DT_REG:
        ...
        for (; p < *pattern_count; p++) {
          ...
          if (last == '$' && first == '^') {
            ...
            if (at_side(0, entry->d_name, arg_len == 1 ? pattern : parsed)) {
              matched = 1;
            }
          } else if (first == '^') {
            /* match at beginning */
            ...
            if (at_side(1, entry->d_name, arg_len == 1 ? pattern : parsed)) {
              matched = 1;
            }
          } else {
            /* substring match */
